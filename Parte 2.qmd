...
aux <- rbind("<0,0001","<0,0001","<0,0001")
...
Nota-se que as variaveis tamanhos dos imóveis tem relação com o nível de renda da população em um nível similar ao com a variável resposta. Esta situação compromete a qualidade do modelo utilizado.
...
for (i in 1:505) {
  if (abs(showcases$lstat[i])>.3) {  
    discrepantes[i] <- discrepantes[i]+1
  }
  if (abs(showcases$ImóveisMenores[i])>.3) {  
    discrepantes[i] <- discrepantes[i]+1
  }
  if (abs(showcases$lstat.ImóveisMenores[i])>.3) {  
    discrepantes[i] <- discrepantes[i]+1
  }
}
```
...
plot_dffits <- function (model, print_plot = TRUE) 
{
    ...
    p <- ggplot(d, aes(x = obs, y = dbetas, label = txt, ymin = 0, 
        ...
        ylab("DFFITS") + ggtitle(paste("Diagnóstico de Influência", 
        title)) + geom_text(hjust = -0.2, nudge_x = 0.15, size = 3, 
        family = "serif", fontface = "italic", colour = "darkred", 
...
Considerando apenas as observações com 3 ou mais ocorrências temos a Tabela a seguir.
...
principais <- principais %>% filter(discrepantes>2)
...
r2a <- round(as.numeric(aux$aju.r.squared),3)

```
Essas observaçãoes correspondem a 6,7% do total de observações do conjunto, desta forma entende-se que a eliminação delas não provocará uma perda significativa de informação para o modelo.  
Desta forma, obteve-se o seguinte modelo resultante:  


$Y_{i}=$ `r aux$coefficients[1,1]` $+$ `r aux$coefficients[2,1]` $X_{1i}=$ `r aux$coefficients[3,1]` $X_{2i}+$ `r aux$coefficients[4,1]` $X_{1,2i}$ 
