...
Os dados de preços de 506 casas em Boston, publicados em Harrison, D.
and Rubinfeld, D.L.
[*'Hedonic prices and the demand for clean air'*](https://www.researchgate.net/profile/Daniel-Rubinfeld/publication/4974606_Hedonic_housing_prices_and_the_demand_for_clean_air/links/5c38ce85458515a4c71e3a64/Hedonic-housing-prices-and-the-demand-for-clean-air.pdf), J.
Environ.
...
O objetivo é o valor médio das casas em um local (em k\$).
...
4.  CHAS: Variável fictícia categórica que representa imóveis próximos a margem do rio Charles (1 se o trecho margeia o rio; 0 caso contrário).
    Será representada como **Margem**.

5.  NOX: Concentração de óxidos nítricos em pphm (partes por 100 milhões).
...
7.  AGE: Proporção de unidades próprias construídas antes de 1940.
    A idade da unidade geralmente está relacionada à qualidade da estrutura.
...
Pode-se verificar pela Figura 3 que cerca de 2/3 das variáveis apresentam valores atípicos (*outlayers*), entretanto o tratamento destes *outlayers* em todos os casos parece, salvo melhor juízo, ser o mesmo.
...
Na avaliação da Figura 4, observa-se que nenhuma das variáveis tem uma aparente forte correlação com o valor médio dos imóveis.
...
fit_anova %>%
  ...
  # )|>
  footnote(
    number = c("Linha 1: Dados referentes a Beta 0", "Linha 2: Dados referentes a Beta 1"),
    number_title = "Legenda:",
    footnote_as_chunk = F
  )|>
  kable_styling(
...
O gráfico que avalia a Normalidade dos Resíduos, Sub.Fig.
...
O gráfico que avalia a Homogeneidade de Variâncias, Sub.Fig.
C, mostra que a Variância não é constante.
...
Embora o banco de dados estudado apresente duas variáveis categóricas: a CHAS, que indica a se o imóvel se encontra ou não as margens do rio Charles e RAD, que representa o índice de acessibilidade às rodovias radiais, verificou-se que a correlação destas variáveis com o preço médio dos imóveis é muito pequena.
...
#| results: false
#| echo: false
m_nul1 <- lm(medv ~ 1, data=aux1)
m_nul2 <- lm(medv ~ 1, data=aux2)
m_coi <- lm(medv ~ lstat, data=dados2)
m_par <- lm(medv ~ lstat + Imóveis, data=dados2)
m_con <- lm(medv ~ lstat * Imóveis, data=dados2)

d_nul1 <- predict(m_nul1,as.data.frame(aux1))
aux1 <- cbind(aux1,d_nul1)
aux2 <- cbind(aux2,d_nul1=NA)
d_nul2 <- predict(m_nul2,as.data.frame(aux2))
aux2 <- cbind(aux2,d_nul2)
aux1 <- cbind(aux1,d_nul2=NA)
d_coi <- predict(m_coi,dados2)
d_par <-  predict(m_par,dados2)
d_con <-  predict(m_con,dados2)
dados2 <- rbind(aux1,aux2)
dados2 <- cbind(dados2,d_coi,d_par,d_con)
...
# "#F1A340" "#998EC3"
  ...
  d1 <- dados2 |>
    ...
    geom_smooth(aes(y=d_nul1), col= "#1F78B4", method=lm, se=TRUE, size = 1)+
    geom_smooth(aes(y=d_nul2), col= "#A6CEE3", method=lm, se=TRUE, size = 1)+
...
A sensível diferença entre as inclinações das retas propostas pelo modelo deixam claro os efeitos multiplicativos da interação entre as variáveis.
Pode-se verifiar assim que o modelo de retas concorrentes foi o mais adequado para descrever o comportamento das variáveis selecionadas.
Com base neste resultado, este modelo será avaliado a seguir.

## Análise dos testes F para modelos encaixados

### Significância do Modelo

Após o ajuste do modelo de restas concorrentes existe a necessidade de se avaliar a significância do mesmo, o teste de hipótese para tal situação será realizado, contendo as seguintes hipóteses:

$$H_0: \hat{\beta_1}= 0$$

$$H_1: \hat{\beta_1} \neq 0.$$ As Tabelas 6 e 7 trazem os principais resultados da tabela ANOVA e do Intervalo de Confiança para os parâmetros, possibilitando assim inferir sobre o modelo ajustado.
...
fit_anova <- MyAnova(m_con, 3) |> as.data.frame()
...
resultados <- cbind(confint(m_con))
rownames(resultados) <- c("$\\hat \\beta_0$", "$\\hat \\beta_1$", "$\\hat \\beta_2$", "$\\hat \\beta_{1,2}$")
...
Através dos Intervalos de Confiança calculados (Tabela 7) é possível afirmar com 95% de confiança que o verdadeiro valor de $\beta_0$ está entre `r glue::glue('({scales::number(confint(m_con)[1,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(m_con)[1,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')`; que o verdadeiro valor de $\beta_1$ está entre `r glue::glue('({scales::number(confint(m_con)[2,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(m_con)[2,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')`; que o verdadeiro valor de $\beta_2$ está entre `r glue::glue('({scales::number(confint(m_con)[3,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(m_con)[3,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')` que o verdadeiro valor de $\beta_{1,2}$ está entre `r glue::glue('({scales::number(confint(m_con)[4,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(m_con)[4,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')`.
...
-   Harrison, David & Rubinfeld, Daniel.
    (1978).
